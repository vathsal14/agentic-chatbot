...
class ResponseAgent(BaseAgent):
    ...
    def _format_context(self, context: Any) -> str:
        ...
        if isinstance(context, list):
            formatted = []
            for i, item in enumerate(context[:3], 1):  # Limit to top 3 contexts
    ...
    async def handle_generate_response(self, message: MCPMessage) -> None:
        ...
        try:
            ...
            max_length = min(int(message.payload.get('max_length', 500)), 1000)  # Cap at 1000 tokens
            ...
            # Extract only the assistant's response (after the last 'ANSWER:')
